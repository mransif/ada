...
function App() {
  ...
  const startRecognition = useCallback(() => {
    console.log(
      `DEBUG: Attempting startRecognition. Conditions - isListening: ${isListeningRef.current
      }, recognition.current: ${!!recognition.current}, isMuted: ${isMutedRef.current
  ...
  useEffect(() => {
    ...
    const handleError = (event) => {
      ...
      } else if (event.error === "network") { // <-- Add specific case for network error
        setStatusText("Network Error: Could not reach speech service. Check connection.");
        userRequestedStop.current = true; // Prevent restart
      }
